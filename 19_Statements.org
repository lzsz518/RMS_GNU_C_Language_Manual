...
在GNU C中，你可以在任何嵌套的块作用域中声明局部标签。局部标签可以像普通标签一样在goto语句中使用，但你只能在声明它的块内部引用它。
...
#+begin_src c
  __label__ label, label2, ...;
#+end_src

局部标签的声明必须放在块的开头，即在任何普通声明或语句之前。

局部标签功能对于复杂的宏很有用。如果一个宏包含了嵌套的循环，使用goto可以方便地跳出这些循环。然而，普通标签的作用域是整个函数，
不能用于此目的：如果宏可以在一个函数中被多次展开，那么该标签在该函数中将被多次定义。局部标签避免了这个问题。例如：
...
普通标签在定义它们的函数中可见，仅限于该函数。然而，明确声明的块内局部标签在该块内声明的嵌套函数中可见。有关详细信息，请参阅嵌套函数。

* 作为值的标签

在GNU C中，你可以使用一元运算符'&&'获取当前函数中定义的标签（或者包含函数中定义的局部标签）的地址。该值的类型为void *。
...
要使用这些值，需要一种跳转到指定位置的方法。这可以通过计算跳转语句`goto *exp;`来实现。例如：
...
标签值常量的一个用途是用它们初始化一个静态数组，作为一个跳转表（jump table）的功能：
...
请注意，上面数组的访问并不检查索引是否在合法的范围内，因为C中的数组索引永远不会进行边界检查。


你可以通过将一个标签减去另一个标签，使表项变成偏移量而不是地址。下面是一个例子：
...
在共享库中，使用偏移量更合适，因为它避免了对数组元素进行动态重定位的需要；因此，该数组可以是只读的。

标签值或偏移量的数组的用途与switch语句类似。switch语句更清晰，因此在可行的情况下优先使用switch语句。

标签值的另一个用途是在线程代码的解释器中。解释器函数中的标签可以存储在线程代码中，以实现超快速的调度。

** 标签值的注意事项

跳转到另一个函数中定义的标签是行不通的，这可能导致不可预测的结果。

避免这种情况的最佳方法是仅将标签值存储在自动变量中，或者在函数内部声明的静态变量的名称中。永远不要将它们作为参数传递。

一个被称为克隆（cloning）的优化技术会生成函数代码的多个简化变体，用于特定的固定参数。在某些情况下，使用标签值，
例如在一次函数调用中保存地址并在另一次函数调用中再次使用，会导致克隆生成错误的结果。这些函数必须禁用克隆。

对该函数进行内联调用也会导致代码的多个副本，每个副本都有名字相同的标签值。在计算跳转中使用标签是没有问题的，
因为计算跳转会禁止内联。然而，在其他方面使用标签值，例如指示错误发生位置，会被错误地进行优化。这些函数必须禁用内联。

要防止函数的内联或克隆，请在其定义中使用 *__attribute__((noinline,noclone))* 。参见属性（Attributes）。

当一个函数在静态变量的初始化器中使用标签值时，它会自动防止函数的内联或克隆。

* 表达式中的语句和声明

在GNU C中，用括号括起来的代码块可以作为表达式使用。这为在表达式中使用局部变量、循环和开关提供了一种方式。我们称之为语句表达式。

回想一下，一个块是由大括号括起来的一系列语句。在这种结构中，圆括号放在大括号的周围。例如：
...
这是一个有效的（尽管比必要的稍微复杂一些）求取foo()绝对值的表达式。

代码块中的最后一条语句应该是一个表达式语句，即一个表达式后面跟着一个分号。该表达式的值将作为语句表达式的值。
如果最后一条语句是其他类型的语句，则语句表达式的值为void。

这个特性主要用于确保宏定义中的每个操作数只计算一次。请参阅宏和自动类型（Macros and Auto Type）以了解更多信息。

在必须是常量的表达式中，例如枚举器的值、位域的宽度或静态变量的初始值，不允许使用语句表达式。

使用goto跳转进入语句表达式或者在switch语句中使用语句表达式是错误的。在语句表达式中跳转（使用goto）或在语句表达式外部使用switch语句都是错误的。
使用计算跳转（参见标签作为值）时，编译器无法检测到错误，但仍然无法正常工作。

允许跳出语句表达式，但由于C中的子表达式计算顺序不是严格确定的，因此无法预测在此之前哪些其他子表达式已经计算过了。例如：

#+begin_src c
  foo (), (({ bar1 (); goto a; 0; }) + bar2 ()), baz();
#+end_src

在跳转之前，它调用了foo和bar1，并且从未调用baz，但可能会调用bar2。如果bar2被调用，那将发生在foo之后、bar1之前。
